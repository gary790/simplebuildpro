...
deployRoutes.post('/', async (c) => {
  ...
  if (limits.deploysPerMonth !== -1) {
    ...
    const [{ count: deployCount }] = await db
      .select({ count: count() })
      .from(deployments)
      .where(and(eq(deployments.projectId, projectId), eq(deployments.createdBy, session.userId)));
  ...
  if (!version)
    throw new AppError(404, 'VERSION_NOT_FOUND', 'Build version not found. Run a build first.');
...
deployRoutes.post('/:projectId/domains/:domainId/verify', async (c) => {
  ...
  try {
    ...
    const cnameVerified = cnameRecords.some(
      (r) => r === expectedCname || r === `${expectedCname}.`,
    );

    // Check TXT record
    const txtRecords = await dnsPromises
      .resolveTxt(`_simplebuildpro.${domainRecord.domain}`)
      .catch(() => []);
