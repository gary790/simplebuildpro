...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((user as any).totpEnabled) {
        // Store temporary data and redirect to MFA verification
        sessionStorage.setItem('mfa_user_id', user.id);
        sessionStorage.setItem('mfa_access_token', tokens.accessToken);
  ...
  const handleOAuth = (provider: 'google' | 'github') => {
    window.location.href = `${API_URL}/api/v1/oauth/${provider}`;
  };
